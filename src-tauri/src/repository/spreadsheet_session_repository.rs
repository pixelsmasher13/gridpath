...
/// Replace the session's attached reference-workbook paths (JSON array of
/// absolute paths). A no-op if the session row doesn't exist yet — the
/// frontend re-persists after the first-prompt upsert.
pub fn set_session_references(
    db: &Connection,
    id: &str,
    reference_paths: &str,
) -> Result<(), rusqlite::Error> {
    db.execute(
        "UPDATE spreadsheet_sessions SET reference_paths = @refs, updated_at = datetime('now') WHERE id = @id",
        named_params! { "@id": id, "@refs": reference_paths },
    )?;
    Ok(())
}

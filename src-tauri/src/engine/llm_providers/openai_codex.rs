    ...
    F: FnMut(StreamEvent) + Send,
{
    ...
    // Set once we observe a clean `response.completed`. If the stream ends or
    // breaks without this, the turn is incomplete and we MUST surface an error
    // rather than returning Ok — otherwise the agent loop reads a dropped
    // connection as "the model chose to stop" and silently ends the turn.
    ...
    while let Some(ev) = stream.next().await {
        let event = match ev {
            ...
            Err(e) => {
                ...
                let msg = format!(
                    "Codex stream interrupted after {} events ({:?} since start, {:?} since last event '{}'): {}",
                    events_received,
                ...
                warn!("{}", msg);
                // Propagate as an error: the stream died before
                // `response.completed`, so the assistant turn is incomplete.
                // Returning Ok here would let the agent loop mistake a dropped
                // connection for a clean stop.
                return Err(msg);
            }
        };
        events_received += 1;
        ...
        match kind {
            ...
            // Codex doesn't expose raw chain-of-thought, but the summary is the
            // legible plan we surface in the UI. Reuse the shared ReasoningDelta
    ...
    // The stream ended without an error event but also without
    // `response.completed` — e.g. the server closed the connection cleanly
    // mid-response. Treat as a failed turn so the loop can surface/retry it.
    if !saw_completed {
        return Err(format!(
            "Codex stream ended without completion after {} events ({:?} since start, last event '{}')",
            events_received,
            std::time::Instant::now().duration_since(stream_started),
            last_event_type,
        ));
    }

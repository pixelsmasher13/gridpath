...
#[tauri::command]
pub async fn spreadsheet_session_set_references(
    app_handle: AppHandle,
    id: String,
    reference_paths: String,
) -> Result<(), String> {
    info!("session_set_references: id={} refs={}", id, reference_paths);
    app_handle
        .db(|db| repo::set_session_references(db, &id, &reference_paths))
        .map_err(map_err)
}

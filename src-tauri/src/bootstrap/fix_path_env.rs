...
/// The error that might happen on a [`fix`] call. The non-`Shell` variants are
/// only ever constructed on non-Windows targets; `#[allow(dead_code)]` keeps
/// Windows builds warning-free without splitting the enum per-platform.

...
// Auth commands
//
// The frontend signs in with Firebase, exchanges the Firebase ID token for a
// proxy JWT at the Vercel auth endpoint, and stashes that JWT here via
// `authenticate_user`. The JWT isn't actually consumed by the spreadsheet
// agent (which talks to Anthropic / OpenAI directly with the user's own
// credentials), so this is purely an account-tracking record at the moment.
// =============================================================================
...
async fn authenticate_user(
    ...
    let auth_token = password; // The "password" field carries the proxy JWT.
    ...
    app_handle
        ...
        .map_err(|e| format!("save user_id: {}", e))?;
...
fn logout_user(app_handle: AppHandle) -> Result<(), String> {
    let user_id = app_handle
        .db(|db| get_setting(db, "user_id").map(|s| s.setting_value).ok())
        .unwrap_or_default();
    if !user_id.is_empty() {
        app_handle
            .db(|db| user_auth_repository::clear_user_auth(db, &user_id))
            .map_err(|e| format!("clear auth: {}", e))?;
        app_handle
            .db(|db| {
                insert_or_update_setting(
                    db,
            ...
            .map_err(|e| format!("clear user_id: {}", e))?;
    }
    Ok(())
}
...
async fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            install_update,
            relaunch_app,
            // Auth (Firebase + proxy JWT bookkeeping)
            authenticate_user,
            logout_user,
            ...
            // Eval driver (GRIDPATH_EVAL_* env — see engine/eval_mode.rs)
            crate::engine::eval_mode::eval_config,
            crate::engine::eval_mode::eval_finish,
            ...
            crate::engine::workbook::commands::save_workbook_patched,
            crate::engine::workbook::commands::save_workbook_patched_as,
            // IronCalc shadow calc engine (spike)
            crate::engine::workbook::calc_engine::calc_engine_load,
            crate::engine::workbook::calc_engine::calc_engine_edit,
            crate::engine::workbook::calc_engine::calc_engine_edit_batch,
            crate::engine::workbook::calc_engine::calc_engine_snapshot,
            crate::engine::workbook::calc_engine::calc_engine_structural,
            crate::engine::workbook::calc_engine::calc_engine_resync,
            crate::engine::workbook::calc_engine::calc_engine_new,
            crate::engine::workbook::calc_engine::calc_engine_sheet_op,
            crate::engine::workbook::calc_engine::calc_engine_get_cell,
            crate::engine::workbook::calc_engine::calc_engine_status,
            crate::engine::workbook::calc_engine::calc_engine_unload,
            // Spreadsheet agent
            ...
            crate::engine::spreadsheet_agent::commands::spreadsheet_tool_started,
            ...
            crate::engine::workbook::session_commands::spreadsheet_session_set_references,
            ...
            crate::engine::workbook::settings_commands::ssws_get_effort,
            crate::engine::workbook::settings_commands::ssws_set_effort,
            crate::engine::workbook::settings_commands::validate_llm_credential,
        ])
        .manage(AppState { db: Default::default() })
        .manage(crate::engine::workbook::calc_engine::CalcEngineState::default())
        ...
        .setup(move |app| {
            ...
            // One-shot migration: ChatGPT subscriptions cannot use `gpt-5-codex`,
            // `gpt-5`, or bare `gpt-5.6` / Sol (the Codex /responses endpoint
            // rejects them with HTTP 400 on Plus/Pro). Older builds shipped these
            // as defaults and persisted them — clear any known-bad stored value
            // so the new default (gpt-5.6-terra) kicks in.
            if matches!(m_openai_codex.as_str(), "gpt-5-codex" | "gpt-5" | "gpt-5.6" | "gpt-5.6-sol") {
                info!(
            ...
            let claude_effort = app_handle.db(|db| {
                get_setting(db, "claude_effort")
                    .map(|s| s.setting_value)
                    .unwrap_or_default()
            });
            crate::engine::provider_config::set_claude_effort(&claude_effort);

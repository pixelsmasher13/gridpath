-- Fix billing rate: production has v_rate_per_minute = 17 ($10.20/hr)
-- Should be 9 (~$5.40/hr, rounded from exact $5/hr = 8.33¢/min)
--
-- Run against production database to correct the rate.
...
DECLARE
  ...
  v_rate_per_minute INTEGER := 9; -- $5/hour = ~$0.09/minute (rounded up from 8.33)
...
BEGIN
  ...
  -- Free minutes are one-time only (no monthly reset)

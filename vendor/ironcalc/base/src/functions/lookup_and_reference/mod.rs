...
impl<'a> Model<'a> {
    ...
    fn as_vector(
        ...
        value: CalcResult,
        cell: CellReferenceIndex,
    ) -> Result<std::sync::Arc<Vec<CalcResult>>, CalcResult> {
        match value {
    ...
    pub(crate) fn fn_match(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        // Memoize (range, match_type, target) for the current recalc pass:
        // Canalyst-style sheets repeat identical MATCH lookups across whole
        // rows/columns of cells. The position is cached, not the CalcResult,
        // because #N/A errors carry a per-caller origin.
        let memo_key = if let CalcResult::Range { left, right } = &match_range {
            crate::model::lookup_cache_key(&target).map(|t| {
                (
                    left.sheet,
                    left.row,
                    left.column,
        ...
        if let Some(key) = &memo_key {
            if let Some(cached) = self.match_cache.get(key) {
                return match cached {
                    Some(pos) => CalcResult::Number(*pos),
        ...
        let position = match_position(&target, &values, match_type);
        if let Some(key) = memo_key {
            self.match_cache.insert(key, position);
        }
        match position {
            Some(pos) => CalcResult::Number(pos),
            None => CalcResult::Error {
    ...
    pub(crate) fn fn_lookup(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        if args.len() == 3 {
            ...
            return result_vector.get(l).cloned().unwrap_or(not_found);
        }
    ...
    /// Resolve an INDIRECT string argument as a defined name.
    ///
    /// A bare name (`"Revenue"`) is looked up in the calling sheet's scope
    /// first, then workbook scope — Excel's resolution order. A qualified
    /// name (`"Sheet2!Revenue"`, `"'My Sheet'!Revenue"`) is looked up in the
    /// named sheet's scope, then workbook scope (Excel accepts a
    /// workbook-level name qualified with any existing sheet).
    fn resolve_indirect_defined_name(
        &self,
        text: &str,
        current_sheet: u32,
    ) -> Option<ParsedDefinedName> {
        let text = text.trim();
        let (scope_sheet, name) = match text.rsplit_once('!') {
            Some((sheet_part, name_part)) => {
                let sheet_part = sheet_part.trim();
                let sheet_name = if sheet_part.len() >= 2
                    && sheet_part.starts_with('\'')
                ...
                (self.get_sheet_index_by_name(&sheet_name)?, name_part)
            }
            None => (current_sheet, text),
        };
        ...
        self.parsed_defined_names
            .get(&(Some(scope_sheet), key.clone()))
            .or_else(|| self.parsed_defined_names.get(&(None, key)))
            .cloned()
    }
        ...
        match value {
            Ok(s) => {
                ...
                let parsed_reference = match parsed_reference {
                    ...
                    Err(message) => {
                        // Excel's INDIRECT also resolves defined names
                        // (`INDIRECT("MyName")`, `INDIRECT("Sheet2!LocalName")`),
                        // which the A1 reference parser doesn't know about.
                        match self.resolve_indirect_defined_name(&s, cell.sheet) {
                            ...
                            // A lambda or malformed name is not a reference.
                            _ => {
                                return CalcResult::Error {
                                    error: Error::REF,
                                    origin: cell,
                ...
                match parsed_reference {
                    ...
                    ParsedReference::Range(left, right) => {
                        self.record_support_range(cell, left, right);
                        CalcResult::Range { left, right }
                    }
                }
            }
...
/// The MATCH search over a materialized vector, returning the 1-based
/// position (or None for #N/A). Split out of `fn_match` so the result can be
/// memoized per (range, match_type, target).
fn match_position(target: &CalcResult, values: &[CalcResult], match_type: i32) -> Option<f64> {
    match match_type {
        -1 => {
            // Binary search leftmost for value in the vector
            let mut l = 0;
            ...
            // l is the number of elements >= target; 0 means below minimum.
            if l == 0 {
                None
            } else {
                Some(l as f64)
        ...
        0 => {
            ...
            let result_matches: Box<dyn Fn(&CalcResult) -> bool> =
                if let CalcResult::String(s) = target {
                    ...
                    let t = target.clone();
                    Box::new(move |x| values_are_equal(x, &t))
                };
            for (l, value) in values.iter().enumerate() {
                if result_matches(value) {
                    return Some(l as f64 + 1.0);
                }
            }
            None
        }

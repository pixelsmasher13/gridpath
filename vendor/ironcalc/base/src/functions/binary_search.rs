...
impl<'a> Model<'a> {
    ...
    /// Cached per recalculation pass: safe because the first materialization
    /// forces lazy evaluation of any dirty cells in the span, after which
    /// their values are fixed for the rest of the pass.
    pub(crate) fn prepare_array(
        &mut self,
        left: &CellReferenceIndex,
        right: &CellReferenceIndex,
        is_row_vector: bool,
    ) -> std::sync::Arc<Vec<CalcResult>> {
        let cache_key = (
            left.sheet,
            left.row,
            left.column,
        ...
        // Clamp the tail of the span to the used area of the sheet: cells
        // beyond it hold no data and evaluate to EmptyCell, but a whole-column
        // reference (G:G) would otherwise materialize a million cells per call.
        let mut right_row = right.row;
        let mut right_column = right.column;
        if let Ok(ws) = self.workbook.worksheet(left.sheet) {
            if is_row_vector {
                let max_row = ws.sheet_data.keys().max().copied().unwrap_or(left.row);
                right_row = right_row.min(max_row);
            } else if let Some(row_data) = ws.sheet_data.get(&left.row) {
                let max_column = row_data.keys().max().copied().unwrap_or(left.column);
                right_column = right_column.min(max_column);
            } else {
        ...
        self.range_cache.insert(cache_key, result.clone());
        result
    }
}

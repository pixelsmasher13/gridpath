...
pub struct Parser<'a> {
    ...
    /// Lowercased-name index into `defined_names` — identifier resolution
    /// runs for every parsed name against potentially thousands of defined
    /// names; the linear scan dominated import on analyst models.
    defined_names_index: HashMap<String, Vec<usize>>,
...
impl<'a> Parser<'a> {
    fn build_defined_names_index(defined_names: &[DefinedNameS]) -> HashMap<String, Vec<usize>> {
        let mut index: HashMap<String, Vec<usize>> = HashMap::new();
        for (i, (name, _, _)) in defined_names.iter().enumerate() {
            index.entry(name.to_lowercase()).or_default().push(i);
        }
        index
    }
    ...
    pub fn new(
        ...
        let defined_names_index = Self::build_defined_names_index(&defined_names);
        Parser {
            lexer,
            worksheets,
            defined_names,
    ...
    pub fn set_worksheets_and_names(
        ...
        self.defined_names_index = Self::build_defined_names_index(&defined_names);
    ...
    fn get_defined_name(&self, name: &str, sheet: u32) -> Option<(Option<u32>, String)> {
        // Indexed lookup: identifier resolution runs for every parsed name;
        // the former linear scan over every defined name dominated import on
        // analyst models. Candidate lists preserve insertion order, so the
        // sheet-local-then-global resolution order is unchanged.
        let candidates = self.defined_names_index.get(&name.to_lowercase())?;
        for &i in candidates {
            let (_, df_scope, df_formula) = &self.defined_names[i];
            if df_scope == &Some(sheet) {
        ...
        match next_token {
            ...
            TokenType::Ident(name) => {
                ...
                for table_name in self.tables.keys() {
                    if crate::functions::util::str_eq_ci(table_name, &name) {

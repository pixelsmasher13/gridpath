...
pub struct Model<'a> {
    ...
    /// Per-recalculation-pass cache of materialized range vectors, keyed by
    /// (sheet, r1, c1, r2, c2, is_row_vector). A label column MATCHed by 500
    /// different cells materializes once instead of 500 times. Cleared at
    /// pass start and at the spill-phase boundary (spills rewrite cells).
    pub(crate) range_cache: HashMap<(u32, i32, i32, i32, i32, bool), Arc<Vec<CalcResult>>>,
    /// Per-pass MATCH memo: (sheet, r1, c1, r2, c2, match_type, target) →
    /// found position (None = #N/A). Canalyst-style sheets repeat identical
    /// MATCH lookups across whole rows/columns of cells.
    pub(crate) match_cache: HashMap<(u32, i32, i32, i32, i32, i32, LookupKeyValue), Option<f64>>,
    /// Lazily-filled: (sheet, formula_index) → the parsed formula contains an
    /// unknown function or a parse error. Such cells keep their imported
    /// cached value instead of being recomputed into #NAME?. Session-safe:
    /// structural edits cannot create these node kinds. Cleared when
    /// formulas are reparsed.
    pub(crate) unevaluable_cache: HashMap<(u32, i32), bool>,
    /// Cells whose IMPORTED formula contains an unresolvable reference
    /// ('[2]Workbook'! externals): they keep their cached value. Import-time
    /// only — a reference that becomes wrong through an in-session edit
    /// (deleting a sheet) must show #REF!, and structural edits shift
    /// coordinates — so this set is CLEARED on any structural change.
    pub(crate) import_wrongref_frozen: std::collections::HashSet<(u32, i32, i32)>,
}

/// Hashable form of a lookup target for the per-pass MATCH memo. Strings are
/// lowercased because MATCH compares case-insensitively.
#[derive(PartialEq, Eq, Hash, Clone)]
pub(crate) enum LookupKeyValue {
    Number(u64),
    Text(String),
    Boolean(bool),
}

pub(crate) fn lookup_cache_key(target: &CalcResult) -> Option<LookupKeyValue> {
    match target {
        CalcResult::Number(n) => Some(LookupKeyValue::Number(n.to_bits())),
        CalcResult::String(s) => Some(LookupKeyValue::Text(s.to_lowercase())),
        CalcResult::Boolean(b) => Some(LookupKeyValue::Boolean(*b)),
        _ => None,
    }
}

/// A formula containing any of these nodes cannot be meaningfully evaluated:
/// * `NamedFunctionKind` — an unknown function: external add-in calls
///   (`_xll.FDS`, `BDP`, `CIQ`, ...) that only the vendor add-in can compute,
/// * `WrongReferenceKind` / `WrongRangeKind` — unresolvable references, e.g.
///   `'[2]Workbook'!A1` externals,
/// * `ParseErrorKind` — syntax the parser could not consume.
/// Cells whose formulas match keep their imported cached value (see
/// `evaluate_cell`); a literal `ErrorKind` (`=#N/A`) is evaluable and is NOT
/// in this class.
fn node_is_unevaluable(node: &Node) -> bool {
    use Node::*;
    match node {
        NamedFunctionKind { .. } | ParseErrorKind { .. } => true,
        WrongReferenceKind { .. } | WrongRangeKind { .. } => false,
        OpRangeKind { left, right }
        | OpConcatenateKind { left, right }
        | OpSumKind { left, right, .. }
        | OpProductKind { left, right, .. }
        | OpPowerKind { left, right }
        | CompareKind { left, right, .. } => {
            node_is_unevaluable(left) || node_is_unevaluable(right)
        }
        UnaryKind { right, .. } => node_is_unevaluable(right),
        FunctionKind { args, .. } => args.iter().any(node_is_unevaluable),
        LambdaDefKind { body, .. } => node_is_unevaluable(body),
        LambdaCallKind { lambda, args } => {
            node_is_unevaluable(lambda) || args.iter().any(node_is_unevaluable)
        }
        ImplicitIntersection { child, .. } | SpillRangeOperator { child } => {
            node_is_unevaluable(child)
        }
        BooleanKind(_) | NumberKind(_) | StringKind(_) | ReferenceKind { .. }
        | RangeKind { .. } | ArrayKind(_) | DefinedNameKind(_) | TableNameKind(_)
        | NamedVariableKind { .. } | ErrorKind(_) | EmptyArgKind => false,
    }
}

/// True when the node tree contains an unresolvable reference (see
/// `import_wrongref_frozen`).
fn node_has_wrong_ref(node: &Node) -> bool {
    use Node::*;
    match node {
        WrongReferenceKind { .. } | WrongRangeKind { .. } => true,
        OpRangeKind { left, right }
        | OpConcatenateKind { left, right }
        | OpSumKind { left, right, .. }
        | OpProductKind { left, right, .. }
        | OpPowerKind { left, right }
        | CompareKind { left, right, .. } => {
            node_has_wrong_ref(left) || node_has_wrong_ref(right)
        }
        UnaryKind { right, .. } => node_has_wrong_ref(right),
        FunctionKind { args, .. } | NamedFunctionKind { args, .. } => {
            args.iter().any(node_has_wrong_ref)
        }
        LambdaDefKind { body, .. } => node_has_wrong_ref(body),
        LambdaCallKind { lambda, args } => {
            node_has_wrong_ref(lambda) || args.iter().any(node_has_wrong_ref)
        }
        ImplicitIntersection { child, .. } | SpillRangeOperator { child } => {
            node_has_wrong_ref(child)
        }
        BooleanKind(_) | NumberKind(_) | StringKind(_) | ReferenceKind { .. }
        | RangeKind { .. } | ArrayKind(_) | DefinedNameKind(_) | TableNameKind(_)
        | NamedVariableKind { .. } | ErrorKind(_) | ParseErrorKind { .. }
        | EmptyArgKind => false,
    }
}
...
impl<'a> Model<'a> {
                ...
                if let Ok(Some(parsed_defined_name)) = self.get_parsed_defined_name(name, *scope) {
                    match parsed_defined_name {
                        ParsedDefinedName::CellReference(reference) => {
                            self.record_support_cell(cell, reference);
                            self.evaluate_cell(reference)
                        }
                        ParsedDefinedName::RangeReference(range) => {
                            self.record_support_range(cell, range.left, range.right);
                            CalcResult::Range {
                                left: range.left,
                                right: range.right,
                            }
            ...
            match original_range {
                ...
                None => {
                    ...
                    let coerced = if array_width == 1 && array_height == 1 {
                        ...
                        // Reachable on real workbooks (e.g. a whole-row reference
                        // in a scalar cell that implicit-intersection insertion
                        // missed) — observed on a Canalyst model with a 16384x1
                        // array. Was a debug_assert!, which aborted dev builds;
                        // the #VALUE! below is the correct Excel-visible outcome.
                        if std::env::var("IC_DEBUG").is_ok() {
                            eprintln!(
                                "[ic] array in scalar context at sheet={sheet}, \
                                 row={row}, column={column} ({array_width}x{array_height})"
                            );
                        }
                        FormulaValue::Error {
                            ei: Error::VALUE,
    ...
    pub(crate) fn evaluate_cell(&mut self, cell_reference: CellReferenceIndex) -> CalcResult {
        ...
        match original_cell.get_formula() {
            Some(f) => {
                ...
                // Unevaluable formulas — external add-in calls (_xll.*, BDP,
                // CIQ...), unresolvable references ('[2]Book'! externals),
                // unparseable syntax — keep their imported cached value and
                // behave as constants. Recomputing them can only produce
                // #NAME?/#REF!, destroying real market data the file carries
                // and cascading errors through every dependent. This matches
                // Excel-without-the-add-in showing cached values. Only when
                // there is NO usable cached value do they evaluate (to the
                // honest error).
                if self.formula_is_unevaluable(cell_reference.sheet, f)
                    || self.import_wrongref_frozen.contains(&key)
                {
                    let cached = self.get_cell_value(&original_cell, cell_reference);
                    if !matches!(cached, CalcResult::Error { .. } | CalcResult::EmptyCell) {
                ...
                let result = if let CalcResult::Range { left, right } = result {
                        ...
                        && left.column == right.column
                    {
                        ...
                        // Runtime implicit intersection. Static analysis inserts `@`
                        // where it can prove a subexpression yields a range in scalar
                        // context, but a range whose shape only exists at runtime (a
                        // defined name resolving to a row band, an INDIRECT/OFFSET
                        // result) reaches here unwrapped. Do what Excel does: a row
                        // or column vector collapses to the element aligned with this
                        // cell; anything else is #VALUE!.
                        if left.column == right.column
                            && left.row <= cell_reference.row
                            && cell_reference.row <= right.row
                        {
                            self.evaluate_cell(CellReferenceIndex {
                                sheet: left.sheet,
                            ...
                            && left.column <= cell_reference.column
                            && cell_reference.column <= right.column
                        {
                            self.evaluate_cell(CellReferenceIndex {
                                sheet: left.sheet,
                                row: left.row,
                            ...
                            CalcResult::new_error(
                                Error::VALUE,
                                cell_reference,
                                "Implicit intersection failed".to_string(),
                            )
                        }
                    } else {
                ...
                match result {
                    CalcResult::Array(a) => {
                        ...
                        if !is_array_formula && (array_width != 1 || array_height != 1) {
                            ...
                            if std::env::var("IC_DEBUG").is_ok() {
                                eprintln!(
                                    "[ic] array in scalar context at {cell_reference:?} \
                                     ({array_width}x{array_height})"
                                );
                            }
                            CalcResult::new_error(
                                Error::VALUE,
    ...
    pub(crate) fn get_sheet_index_by_name(&self, name: &str) -> Option<u32> {
        ...
        for (index, worksheet) in worksheets.iter().enumerate() {
            if crate::functions::util::str_eq_ci(&worksheet.get_name(), name) {
    ...
    pub fn from_workbook(workbook: Workbook, language_id: &str) -> Result<Model<'_>, String> {
        ...
        let mut model = Model {
            ...
            unevaluable_cache: HashMap::new(),
            import_wrongref_frozen: std::collections::HashSet::new(),
        };

        model.parse_formulas();
        // Freeze cells whose imported formula carries an unresolvable
        // reference — their cached value is real data (see the field doc).
        let mut frozen = std::collections::HashSet::new();
        for (sheet_index, ws) in model.workbook.worksheets.iter().enumerate() {
            for (row, columns) in &ws.sheet_data {
                for (column, cell) in columns {
                    if let Some(f) = cell.get_formula() {
                        if let Some((node, _)) = model
                            .parsed_formulas
                            .get(sheet_index)
                            .and_then(|fs| fs.get(f as usize))
                        {
                            if node_has_wrong_ref(node) {
                                frozen.insert((sheet_index as u32, *row, *column));
                            }
                        }
                    }
                }
        ...
        model.import_wrongref_frozen = frozen;
    ...
    // ------------------------------------------------------------------
    // GRIDPATH resync support: reseeding a drifted model from the grid.
    // ------------------------------------------------------------------

    /// GRIDPATH: clears every worksheet's cell content, formula storage and
    /// evaluation caches while KEEPING sheets, defined names, styles and
    /// shared strings. First half of a resync-from-grid: the grid is the
    /// source of truth for cell content, but it does not know the
    /// workbook's defined names (the import brought those in and formulas
    /// depend on them), so the model is reseeded in place, never rebuilt
    /// from scratch.
    pub fn resync_clear_contents(&mut self) {
        for ws in self.workbook.worksheets.iter_mut() {
            ws.sheet_data.clear();
            ws.shared_formulas.clear();
        }
        for parsed in self.parsed_formulas.iter_mut() {
            parsed.clear();
        }
        self.cells.clear();
        self.spill_cells.clear();
        self.support.clear();
        self.cf_cache.clear();
        self.range_cache.clear();
        self.match_cache.clear();
        self.unevaluable_cache.clear();
        self.import_wrongref_frozen.clear();
    }

    /// GRIDPATH: seeds a formula cell carrying a cached value — the resync
    /// equivalent of an imported formula cell. `formula` may include the
    /// leading '='. Unevaluable formulas (external add-in calls, parse
    /// errors) freeze at `cached` through the lazy unevaluable cache, and
    /// wrong-reference formulas ('[2]Book'! externals) are registered in
    /// `import_wrongref_frozen` exactly as `from_workbook` does — frozen
    /// market data survives a resync instead of collapsing to #NAME?/#REF!.
    /// Call before `evaluate()`.
    pub fn seed_formula_with_cached_value(
        &mut self,
        sheet: u32,
        row: i32,
        column: i32,
        ...
        let index = self.set_cell_with_formula(sheet, row, column, text, style)?;
        ...
        {
            if node_has_wrong_ref(node) {
                self.import_wrongref_frozen.insert((sheet, row, column));
            }
        }
        if !matches!(cached, FormulaValue::Unevaluated) {
            if let Some(Cell::CellFormula { v, .. } | Cell::ArrayFormula { v, .. }) = self
                .workbook
                .worksheet_mut(sheet)?
                .sheet_data
                .get_mut(&row)
    ...
    /// GRIDPATH: sets a literal cell from a TYPED value, bypassing input
    /// parsing — a grid string like "123" or "TRUE" must stay text through
    /// a resync, and numbers must not round-trip through locale parsing.
    pub fn seed_cell_literal(
        &mut self,
        sheet: u32,
        row: i32,
        column: i32,
        ...
        match value {
            CellValue::None => Ok(()),
            CellValue::String(s) => self.set_cell_with_string(sheet, row, column, &s, style),
            CellValue::Number(n) => self.set_cell_with_number(sheet, row, column, n, style),
            CellValue::Boolean(b) => self.set_cell_with_boolean(sheet, row, column, b, style),
        }
    }
    ...
    fn get_parsed_defined_name(
        ...
        // Keys are stored lowercased (see parse_defined_names): a direct
        // lookup replaces what was a linear scan over every defined name with
        // a case-insensitive comparison each — the top of the evaluation
        // profile on defined-name-heavy analyst models.
        Ok(self
            .parsed_defined_names
    ...
    pub fn evaluate(&mut self) {
        ...
        // Spill evaluation may have rewritten cells cached during phase 1.
        self.range_cache.clear();
        self.match_cache.clear();
    ...
    /// True when the parsed formula for `(sheet, formula_index)` contains a
    /// node the engine cannot meaningfully evaluate. Lazily cached.
    fn formula_is_unevaluable(&mut self, sheet: u32, formula_index: i32) -> bool {
        let key = (sheet, formula_index);
        if let Some(v) = self.unevaluable_cache.get(&key) {
            return *v;
        }
        let v = self
            .parsed_formulas
            .get(sheet as usize)
            .and_then(|fs| fs.get(formula_index as usize))
            .map(|(node, _)| node_is_unevaluable(node))
            .unwrap_or(false);
        self.unevaluable_cache.insert(key, v);
        v
    }

    /// Records that `reader` read `target` during evaluation. Used by INDIRECT,
    /// OFFSET and defined names, whose targets are invisible to the parser and
    /// therefore never flow through the ReferenceKind/RangeKind recording.
    pub(crate) fn record_support_cell(
        &mut self,
        ...
        self.support
            .entry(reader)
            .or_default()
            .push(CellOrRange::Cell((target.sheet, target.row, target.column)));
    }

    pub(crate) fn record_support_range(
        &mut self,
        reader: CellReferenceIndex,
        left: CellReferenceIndex,
        right: CellReferenceIndex,
        ...
        self.support
            ...
            .push(CellOrRange::Range((
                left.sheet,
                left.row.min(right.row),
                left.column.min(right.column),
                left.row.max(right.row),
                left.column.max(right.column),
            )));
    }

    /// The positions (excluding the anchor) written by `anchor` if it is an
    /// array formula, using its current recorded dimensions.
    fn spill_area_of(&self, anchor: (u32, i32, i32)) -> Option<Vec<(u32, i32, i32)>> {
        let ws = self.workbook.worksheet(anchor.0).ok()?;
        if let Some(Cell::ArrayFormula { r, .. }) = ws.cell(anchor.1, anchor.2) {
            let (width, height) = *r;
            let mut area = Vec::new();
            for row in anchor.1..anchor.1 + height {
                for column in anchor.2..anchor.2 + width {
                    if row == anchor.1 && column == anchor.2 {
                        continue;
                    }
                    area.push((anchor.0, row, column));
                }
            }
            return Some(area);
        }
    ...
    /// Incrementally re-evaluates only the cells transitively affected by the
    /// `edited` cells, using the dependency edges recorded during the last
    /// evaluation. Requires a prior full `evaluate()`. Returns the cells whose
    /// values may have changed (the dirty closure plus rewritten spill areas),
    /// so a UI can refresh exactly those.
    ///
    /// Limitations (fall back to a full `evaluate()` for these):
    /// * structural changes (insert/delete rows/columns, sheet renames,
    ///   defined-name redefinition),
    /// * volatile functions (RAND, NOW, TODAY, ...) are not refreshed,
    /// * a spill whose area grows or shrinks may need the full two-phase
    ///   reordering.
    pub fn evaluate_edited(&mut self, edited: &[(u32, i32, i32)]) -> Vec<(u32, i32, i32)> {
        let debug = std::env::var("IC_DEBUG").is_ok();
        let t0 = std::time::Instant::now();
        // Invert the runtime dependency map: dependency -> readers. Range deps
        // are indexed for the closure walk: narrow ranges (lookup columns,
        // small rects) bucket under each covered (sheet, column) so a dirty
        // cell only scans ranges that can contain it; wide rects (whole-row
        // references, big tables) go to a small per-sheet list. A flat list
        // was O(dirty x all-ranges): ~1s on a 9.6k-cell closure.
        const NARROW: i32 = 8;
        let mut point_readers: HashMap<(u32, i32, i32), Vec<CellReferenceIndex>> = HashMap::new();
        let mut col_ranges: HashMap<(u32, i32), Vec<(i32, i32, CellReferenceIndex)>> =
            HashMap::new();
        // Row-band ranges (a defined name like MO_RIS_OtherOPEX = one model row
        // across ~90 columns) bucket under each covered (sheet, row).
        let mut row_ranges: HashMap<(u32, i32), Vec<(i32, i32, CellReferenceIndex)>> =
            HashMap::new();
        let mut wide_ranges: HashMap<u32, Vec<(i32, i32, i32, i32, CellReferenceIndex)>> =
            HashMap::new();
        let mut range_count = 0usize;
        for (reader, deps) in &self.support {
            for dep in deps {
                match dep {
                    CellOrRange::Cell(t) => point_readers.entry(*t).or_default().push(*reader),
                    CellOrRange::Range((s, r1, c1, r2, c2)) => {
                        range_count += 1;
                        if c2 - c1 < NARROW {
                            for c in *c1..=*c2 {
                                col_ranges.entry((*s, c)).or_default().push((*r1, *r2, *reader));
                            }
                        } else if r2 - r1 < NARROW {
                            for r in *r1..=*r2 {
                                row_ranges.entry((*s, r)).or_default().push((*c1, *c2, *reader));
                            }
                        } else {
                            wide_ranges
                                .entry(*s)
                                .or_default()
                                .push((*r1, *c1, *r2, *c2, *reader));
                        }
                    }
                }
            }
        ...
        if debug {
            let wide: usize = wide_ranges.values().map(|v| v.len()).sum();
            eprintln!(
                "[ic] invert: {} ms ({} point targets, {} range deps, {} wide)",
        ...
        let mut dirty: std::collections::HashSet<(u32, i32, i32)> =
            std::collections::HashSet::new();
        // Cells whose value (may have) changed; their readers must re-evaluate.
        let mut frontier: Vec<(u32, i32, i32)> = Vec::new();
        ...
        for &e in edited {
            ...
            let has_formula = self
                ...
                .worksheet(e.0)
                .ok()
                .and_then(|ws| ws.cell(e.1, e.2))
                .and_then(|c| c.get_formula())
                .is_some();
            if has_formula {
                dirty.insert(e);
            }
            if let Some(area) = self.spill_area_of(e) {
                frontier.extend(area);
            }
        }

        while let Some(t) = frontier.pop() {
            let mut hit: Vec<CellReferenceIndex> = Vec::new();
            if let Some(readers) = point_readers.get(&t) {
                hit.extend_from_slice(readers);
            }
            if let Some(list) = col_ranges.get(&(t.0, t.2)) {
                for (r1, r2, reader) in list {
                    if *r1 <= t.1 && t.1 <= *r2 {
                        hit.push(*reader);
                    }
                }
            }
            if let Some(list) = row_ranges.get(&(t.0, t.1)) {
                for (c1, c2, reader) in list {
                    if *c1 <= t.2 && t.2 <= *c2 {
                        hit.push(*reader);
                    }
                }
            }
            if let Some(list) = wide_ranges.get(&t.0) {
                for (r1, c1, r2, c2, reader) in list {
                    if *r1 <= t.1 && t.1 <= *r2 && *c1 <= t.2 && t.2 <= *c2 {
                        hit.push(*reader);
                    }
                }
            }
            for reader in hit {
                let key = (reader.sheet, reader.row, reader.column);
                if dirty.insert(key) {
                    frontier.push(key);
                    // A dirty spill anchor rewrites its whole area.
                    if let Some(area) = self.spill_area_of(key) {
                        frontier.extend(area);
                    }
                }
            }
        ...
        // Dirty spill anchors first, in the order fixed up by the last full
        // evaluation: spill positions hold no formula, so lazy recursion from a
        // dependent cannot reach their anchor.
        let spill_cells = self.spill_cells.clone();
        for sc in &spill_cells {
            if dirty.contains(&(sc.sheet, sc.row, sc.column)) {
                self.evaluate_cell(*sc);
            }
        }
        // Spill re-evaluation may have rewritten cells cached above.
        self.range_cache.clear();
        self.match_cache.clear();
        for key in &dirty {
            self.evaluate_cell(CellReferenceIndex {
        ...
        if debug {
            eprintln!("[ic] re-evaluate: {} ms", t2.elapsed().as_millis());
        }
        let t3 = std::time::Instant::now();
        self.evaluate_conditional_formatting();
        if debug {
            eprintln!("[ic] conditional formatting: {} ms", t3.elapsed().as_millis());
        }
        // Include the spill areas of dirty anchors: their positions were
        // rewritten even though they hold no formula of their own.
        let mut changed: Vec<(u32, i32, i32)> = Vec::with_capacity(dirty.len());
        for key in dirty {
            if let Some(area) = self.spill_area_of(key) {
                changed.extend(area);
            }

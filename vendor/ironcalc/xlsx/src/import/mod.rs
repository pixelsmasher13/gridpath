...
fn load_xlsx_from_reader<R: Read + std::io::Seek>(
    ...
    if debug {
        eprintln!("[ic] shared strings: {} ms", t.elapsed().as_millis());
    }
    let t = std::time::Instant::now();
    ...
    if debug {
        eprintln!("[ic] workbook/styles/theme: {} ms", t.elapsed().as_millis());
    }
    let t = std::time::Instant::now();
    ...
    if debug {
        eprintln!("[ic] load_sheets: {} ms", t.elapsed().as_millis());
    }
...
pub fn load_from_xlsx<'a>(
    ...
    if debug {
        eprintln!("[ic] load_from_excel: {} ms", t0.elapsed().as_millis());
    }
    let t1 = std::time::Instant::now();
    let model = Model::from_workbook(workbook, language).map_err(XlsxError::Workbook);
    if debug {
        eprintln!("[ic] Model::from_workbook: {} ms", t1.elapsed().as_millis());
    }
    model
}

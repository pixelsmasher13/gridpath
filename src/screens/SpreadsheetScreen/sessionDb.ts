...
export type SessionRow = {
  ...
  /**
   * JSON array of absolute paths to reference workbooks attached to this
   * session as read-only agent context (added 2026-07-01). "[]" when none.
   */
...
/**
 * Persist the session's attached reference-workbook paths (JSON-encoded
 * array). No-ops in SQL if the session row doesn't exist yet — submitPrompt
 * re-persists after the first upsert, so nothing is lost.
 */
export async function setSessionReferences(id: string, paths: string[]): Promise<void> {
  await invoke("spreadsheet_session_set_references", {
    id,
    referencePaths: JSON.stringify(paths),
  });
}

...
import { Clock, Archive, Trash2, FilePlus, FolderOpen, Loader2 } from "lucide-react";
...
// Same running indicator as the tab strip, so "agent is working here"
...
interface Props {
  ...
  /** Session ids (= tab ids) whose agent is currently running. */
...
export const SessionSidebar: React.FC<Props> = ({
  sessions, activePath, runningIds, onOpen, onArchive, onDelete, onNewBlank, onOpenExisting,
}) => {
  return (
    <Panel>
      <Header>
        ...
        {sessions.map((s) => {
          ...
          >
            <NameRow>
              ...
              {runningIds.has(s.id) && <Spin size={11} />}
            </NameRow>
            <Fineprint>
              {fineprintLeft} · {formatAgo(s.updated_at)}{runningIds.has(s.id) ? " · agent working" : ""}
            </Fineprint>
            <Actions>
              <IconBtn
